...
function nextState() {
    const state = Flip.getState(".block, .logo-now", {props: "color,backgroundColor", simple: true}); // capture current state
    ...
    if (curLayout < layouts.length -1) {
        ...
        gsap.fromTo(intro, {opacity: 1}, {opacity: 0, duration: 1.25, delay: 1.25, onComplete: () => {
            ...
            document.getElementById("scrolling-container").style.display = "flex";
            document.body.style.background = "radial-gradient(circle, var(--gradient-start-color) 0%, var(--gradient-end-color) 100%)";
        }});
        return;
    }
    ...
    Flip.from(state, { // animate from the previous state
        ...
        simple: true,
        onEnter: (elements, animation) => gsap.fromTo(elements, {opacity: 0}, {opacity: 1, delay: animation.duration() - 0.2}),
        onLeave: elements => {
